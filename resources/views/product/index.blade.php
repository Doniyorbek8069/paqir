                            ...
                            <div class="modal fade" id="modal{{ $loop->index }}" tabindex="-1" aria-labelledby="exampleModalLabel2" aria-hidden="true">
                                ...
                                <form action="{{route('admin.product.update', ['product' => $item->id])}}" method="post">
                                    ...
                                    <div class="modal-body">
                                        <div class="row mb-3">
                                            <div class="col-sm-12">
                                                <input type="text" class="form-control" placeholder="Nomini Kiriting" name="name" value="{{ $item->name }}" required>
                                            </div>
                                        </div>
                                        <p></p>
                                        <div class="row mb-3">
                                            <div class="col-sm-12">
                                                <input type="number" class="form-control" placeholder="Mahsulot Sotuv Narxi" name="price" value="{{ $item->price }}">
                                            </div>
                                        </div>
                                    </div>
...
<div class="modal fade" id="modalstore" tabindex="-1" aria-labelledby="exampleModalLabel2" aria-hidden="true">
    ...
    <form action="{{route('admin.product.store')}}" method="POST">
        @csrf
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel2">Mahsulot Qo'shish</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
            @csrf
            ...
            <div class="row mb-3">
                <div class="col-sm-12">
                    <input type="number" class="form-control" placeholder="Mahsulot Sotuv Narxi" name="price" required>
                </div>
            </div>
        </div>
        <div class="modal-footer">
